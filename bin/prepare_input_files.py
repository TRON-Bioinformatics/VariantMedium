...
def read_samplesheet(path):
    ...
    if "," in text and "\t" not in text:
        ...
        raise ValueError(
            "Cannot detect delimiter. File must be comma OR tab separated."
        )
        ...
        if skip_preprocessing.lower() != "true":
            if not tumor.with_suffix(".bai").exists():
                raise FileNotFoundError(
                    f"Missing tumor BAI: {tumor.with_suffix('.bai')}"
                )
            if not normal.with_suffix(".bai").exists():
                raise FileNotFoundError(
                    f"Missing normal BAI: {normal.with_suffix('.bai')}"
                )
...
def make_input(df, skip_preprocessing, output_dir):
    ...
    for _, row in df.iterrows():
        ...
        if skip_preprocessing.lower() != "true":
            tumor = out(
                f"output_01_01_preprocessed_bams/{rep}_tumor/{rep}_tumor.preprocessed.bam"
            )
            normal = out(
                f"output_01_01_preprocessed_bams/{rep}_normal/{rep}_normal.preprocessed.bam"
            )
        ...
        # vcfs.tsv
        strelka_vcf = out(
            f"output_01_02_candidates_strelka2/{rep}/{rep}.strelka2.somatic.vcf.gz"
        )
