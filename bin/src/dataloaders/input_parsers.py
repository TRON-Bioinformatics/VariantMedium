...
def get_merged_df(paths, prediction_mode, for_train, aug_list, unknown_strategy):
...
def get_file_paths(
    input_home: Text, for_train: bool, aug_mixes: List[Text] = None
) -> List[Text]:
    ...
    """
    file_list = glob.glob(
        "{}/purity-1.0-downsample-1.0-contamination-0.0/*.pt".format(input_home)
    )
...
def combine_candidates_and_labels(
    labels_df: pd.DataFrame, candidates_df: pd.DataFrame, unknown_strategy: Text
) -> pd.DataFrame:
    ...
    """
    df = labels_df.merge(candidates_df, how="right", on=["CHROM", "POS", "REF", "ALT"])
...
def parse_variants(path: Text, prediction_mode: Text) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            ...
            raise Exception(
                "Wrong variants file type/format. Please check {}.".format(path)
            )
...
def read_array(
    ...
    :return:
    """
    arr = torch.load(file_path, weights_only=True)
    if clip_length > 0:
...
def clip_array(
    arr: torch.Tensor, width: int, aug_rate: int, clip_length: int
) -> torch.Tensor:
    ...
    zero_out_amount = 0
    if aug_rate > 0:
        zero_out_amount = int((width / (aug_rate * 2)) * clip_length)
