...
from src.filter_candidates.extra_trees_functions import (
    read_and_fit,
    compute_metrics,
    apply_threshold,
)
...
def workflow(
    ...
    features, label, sets, thresholds, tuned_params, muttype = get_params(for_indel)
...
def filter_simple(df):
    df = df[~((df["normal_af"] > 0.05) & (df["normal_dp"] > 20))].reset_index(drop=True)
    df = df[df["normal_ac"] < 3].reset_index(drop=True)
    df = df[df["primary_ac"] > 2].reset_index(drop=True)
    df = df[df["primary_af"] > 0.03].reset_index(drop=True)
...
def workflow_validation(
    ...
    if set_threshold:
        ...
        for th in range(1, 50, 1):
            ...
            df_tmp = apply_threshold(
                clf=clf, df=df_tmp, threshold=th / 1000, features=features, label=label
            )
    ...
    df = apply_threshold(
        clf=clf, df=df, threshold=thresholds[model_name], features=features, label=label
    )
